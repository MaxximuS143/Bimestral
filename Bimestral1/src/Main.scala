object Main {
  ...
  val estudiantes: List[Estudiante] = List(
    ...
    @main def estudianteMasConsistente(): Unit = {

      var estudiantesValidos: List[Estudiante] = Nil
      for (e <- estudiantes) {
        if (e.notas.length >= 3) {
          estudiantesValidos = e :: estudiantesValidos
        }
      }
      if (estudiantesValidos.isEmpty) {
        println("No existe resultado: no hay estudiantes con al menos 3 calificaciones.")
        return
      }
      var mejorEstudiante: Estudiante = estudiantesValidos.head
      var mejorConsistencia: Double = Double.MaxValue

      for (e <- estudiantesValidos) {

        var maxNota = e.notas.head
        var minNota = e.notas.head
        ...
        val consistencia = maxNota - minNota
      ...
      println(s"Estudiante más consistente: ${mejorEstudiante.nombre}")
      println(f"Consistencia: $mejorConsistencia%.2f")
    }
